...
import org.springframework.samples.petclinic.rest.dto.VisitCancelRequestDto;
...
public class VisitRestControllerV1 implements VisitsApi {
    ...
    @PreAuthorize("hasRole(@roles.OWNER_ADMIN)")
    @Override
    public ResponseEntity<VisitDto> cancelVisit(Integer visitId, VisitCancelRequestDto visitCancelRequestDto) {
        if (visitCancelRequestDto == null
            || visitCancelRequestDto.getReason() == null
            || visitCancelRequestDto.getReason().trim().isEmpty()
            || visitCancelRequestDto.getReason().length() > 255) {
        ...
        if (Boolean.TRUE.equals(currentVisit.getCancelled())) {
            return new ResponseEntity<>(HttpStatus.CONFLICT);
        }
        currentVisit.setCancelled(true);
        currentVisit.setCancellationReason(visitCancelRequestDto.getReason());
        this.clinicService.saveVisit(currentVisit);
